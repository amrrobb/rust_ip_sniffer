...
async fn scan(tx: Sender<u16>, start_port: u16, addr: IpAddr) {
    // Attempts connect to the address and the give ports
    match TcpStream::connect(format!("{}:{}", addr, start_port)).await {
        // If the connection succesful, print out a '.' and send port to the channel
        Ok(_) => {
            print!(".");
            stdout().flush().unwrap();
...
async fn main() {
    ...
    // Initialize the channel with explicit type annotations.
